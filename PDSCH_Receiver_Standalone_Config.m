...
PDSCH_Profiles(1).SlotIndices = 2:4;
PDSCH_Profiles(1).RBStart = 0;              % BWP/载波栅格内0-based起始RB
PDSCH_Profiles(1).NumOfRB = NumOfRB;
PDSCH_Profiles(1).MCS_TABLE_PDSCH = 1;
PDSCH_Profiles(1).MCS = 20;
PDSCH_Profiles(1).PDSCH_StartSymbol = 2;    % 1-based
PDSCH_Profiles(1).PDSCH_NumSymbols = 13;    % 连续PDSCH OFDM符号数(一般为14-PDSCH_StartSymbol+1)
%% ---------- DMRS配置 (需与发射端一致) ----------
PDSCH_Profiles(1).DMRS_port = 1;            % DMRS端口数 (1~8)
PDSCH_Profiles(1).NumOfAddDMRS = 0;         % 附加DMRS位置 {0,1,2,3}
...
PDSCH_Profiles(1).nID = 0;                  % dataScramblingIdentityPDSCH
