...
slotOffsetMatrix = frame_start_offsets(:) + ...
    (dlSlots - 1) * samples_per_slot;
slotOffsets = reshape(slotOffsetMatrix.', 1, []);
slotNumbers = repmat(dlSlots, 1, numel(frame_start_offsets));
validSlots = slotOffsets >= 0 & ...
    slotOffsets + samples_per_slot <= numSamples;
slotOffsets = slotOffsets(validSlots);
slotNumbers = slotNumbers(validSlots);
...
fprintf('PDSCH时隙范围: %d帧, slot#%d -> slot#%d (%d个时隙)\n', ...
    numel(frame_start_offsets), min(slotNumbers), max(slotNumbers), ...
    numel(slotOffsets));
